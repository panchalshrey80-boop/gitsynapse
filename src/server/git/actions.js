...
import { expandUserPath, lookLikeGitDirectory } from '../paths.js';
...
/**
 * Reads the identity git would stamp on a commit here: the effective value
 * (local config first, then global) and what is in the global config alone, so
 * the UI can say which one it is about to change.
 */
export async function getIdentity(repoPath = process.cwd()) {
  ...
  const [name, email, globalName, globalEmail] = await Promise.all([
    ...
    read(['config', '--global', '--get', 'user.name']),
    read(['config', '--global', '--get', 'user.email']),
  ]);
  ...
  return {
    ...
    // Git refuses to guess at commit time when neither level has the values,
    // which is exactly the state a freshly installed git is in.
    source: globalName && globalEmail ? 'global' : (name && email ? 'local' : 'none'),
  };
}

/**
 * Writes the name and email git stamps on commits.
 *
 * Global by default: the reason a user is here is that git has no identity at
 * all, and setting it per-repository would make them do this again for every
 * project. Values are validated the same way every other git argument is — no
 * option-shaped strings, no NUL bytes, no newlines that could inject a second
 * config line.
 */
export async function setIdentity({ name, email }, { global = true, repoPath = null } = {}) {
  const cleanName = safeValue(String(name ?? ''), 'Your name').trim();
  const cleanEmail = safeValue(String(email ?? ''), 'Your email').trim();

  if (/[\r\n]/.test(cleanName) || /[\r\n]/.test(cleanEmail)) {
    throw new ActionError('Name and email must be a single line each.');
  }
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(cleanEmail)) {
    throw new ActionError('That does not look like an email address. Git records it in every commit, so it is worth getting right.');
  }
  if (cleanName.length > 120 || cleanEmail.length > 190) {
    throw new ActionError('That name or email is implausibly long.');
  }

  const scope = global ? ['--global'] : ['--local'];
  const dir = repoPath || process.cwd();

  for (const [key, value] of [['user.name', cleanName], ['user.email', cleanEmail]]) {
    const result = await runGitIn(dir, ['config', ...scope, key, value]);
    if (!result.ok) throw new ActionError(`Could not save ${key}: ${result.stderr.trim() || 'git refused.'}`, 500);
  }
...
/**
 * Creates a repository in an existing folder.
 *
 * Four things this refuses, each because the alternative is a trap:
 *
 *   - an **empty path**, which `path.resolve('')` turns into the current working
 *     directory. That is how a stray `.git` once appeared in the app's own
 *     install folder: pressing Initialise with the box empty.
 *   - a folder that **does not exist** — git would create it, silently making a
 *     folder the user did not ask for.
 *   - a **bare repository**, which has no working tree. `git init` there reports
 *     success and changes nothing, so the user was left in a loop of "not a
 *     repository" and "initialised".
 *   - a folder **already inside a repository**. Initialising would nest a second
 *     repository inside the first, which is the single easiest way to confuse
 *     someone new to git. The existing root is returned instead, so the caller
 *     can just open it.
 */
export async function initRepository(targetPath, { initialBranch = 'main' } = {}) {
  let absolute;
  try {
    absolute = expandUserPath(targetPath);
  } catch (error) {
    throw new ActionError(error.message, 400);
  }

  if (!fs.existsSync(absolute)) {
    throw new ActionError(`That folder does not exist: ${absolute}. Create it first, then initialise it.`, 404);
  }
  if (!fs.statSync(absolute).isDirectory()) {
    throw new ActionError(`${absolute} is a file, not a folder.`);
  }

  if (lookLikeGitDirectory(absolute) && !fs.existsSync(path.join(absolute, '.git'))) {
    throw new ActionError(
      'That is a bare repository — it holds history but has no working tree, so there are no '
      + 'files to edit. Open the folder of the project you cloned from it instead.',
    );
  }

  const existing = findRepositoryRoot(absolute);
  if (existing) {
    throw new ActionError(
      `That folder is already inside a git repository (${existing.root}). `
      + 'Use "Open a repository" instead — or choose a folder outside it.',
      409,
    );
  }
  ...
  try {
    ...
    throw new ActionError(`GitSynapse cannot write to ${absolute}, so git cannot create a .git folder there.`);
  }
...
export async function cloneRepository(url, destinationParent, { depth = null } = {}) {
  let parent;
  try {
    parent = expandUserPath(destinationParent);
  } catch (error) {
    throw new ActionError(error.message, 400);
  }
