...
import {
  canRead, describeFsError, expandUserPath, isDirectory, lookLikeGitDirectory, PathError,
} from '../paths.js';

// Re-exported: callers have imported isDirectory from this module since 0.1.
export { isDirectory };
...
export function findRepositoryRoot(inputPath) {
  let current;
  try {
    // `~` and relative paths are expanded here rather than at each call site, so
    // every entry point (open, init, clone, the folder browser) behaves alike.
    current = fs.realpathSync(expandUserPath(inputPath));
  } catch {
    return null;
  }
  ...
  // A bare repository is a git directory with no working tree. Saying "not a
  // repository" would be wrong, and offering to initialise one would be worse.
  if (lookLikeGitDirectory(current) && !fs.existsSync(path.join(current, '.git'))) {
    return { root: current, gitDir: current, bare: true };
  }
  ...
  for (let depth = 0; depth < 40; depth += 1) {
    ...
    try {
      ...
      if (stat.isDirectory()) return { root: current, gitDir: gitEntry, bare: false };
      if (stat.isFile()) {
        ...
        if (match) {
          return { root: current, gitDir: path.resolve(current, match[1].trim()), bare: false };
        }
      }
    } catch {
...
export function listDirectories(target) {
  const absolute = expandUserPath(target);

  let entries;
  try {
    entries = fs.readdirSync(absolute, { withFileTypes: true });
  } catch (error) {
    // Without this the route returned an HTML 500 and the browser said
    // "Cannot read that folder: undefined".
    throw describeFsError(error, absolute);
  }
  ...
  const directories = entries
    ...
    .map((entry) => {
      ...
      return {
        ...
        // Surfaced so the browser can explain an empty-looking list rather than
        // silently omitting folders the user can see in Finder.
        blocked: !canRead(full),
      };
    })
    .sort((a, b) => {
...
export async function getStatus(repoPath) {
  const [statusResult, symbolicResult, branchResult, remoteResult, headResult, headExists] = await Promise.all([
    runGitIn(repoPath, ['status', '--porcelain=v1', '-z', '--untracked-files=all', '--branch']),
    // `symbolic-ref` is the only one of these that names the branch of a
    // repository with no commits yet. It fails only on a genuinely detached
    // HEAD, where `rev-parse --abbrev-ref` gives the useful answer instead.
    runGitIn(repoPath, ['symbolic-ref', '--short', 'HEAD']),
    runGitIn(repoPath, ['rev-parse', '--abbrev-ref', 'HEAD']),
    runGitIn(repoPath, ['remote']),
    runGitIn(repoPath, ['rev-parse', '--short', 'HEAD']),
    runGitIn(repoPath, ['rev-parse', '--verify', '--quiet', 'HEAD']),
  ]);

  // A repository created a moment ago has a branch and no commits. Asking
  // `rev-parse --abbrev-ref HEAD` about it fails with exit 128, which the app
  // used to read as "branch named HEAD", and then flagged as a detached HEAD —
  // so every freshly initialised repository looked broken the moment it opened.
  const unborn = !headExists.ok;
  const branch = symbolicResult.ok
    ? symbolicResult.stdout.trim()
  ...
  return {
    ...
    // Detached means "HEAD is not on a branch", which is not the same as "this
    // branch has no commits yet".
    detached: !symbolicResult.ok,
    unborn,
    files,
...
export async function currentBranch(repoPath) {
  // Same reasoning as getStatus: a repository with no commits has a branch
  // name, and only `symbolic-ref` will tell us what it is.
  const symbolic = await runGitIn(repoPath, ['symbolic-ref', '--short', 'HEAD']);
