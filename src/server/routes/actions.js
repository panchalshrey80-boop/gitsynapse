...
import { explainGitFailure } from '../git/failures.js';
...
const REPO_FREE_ACTIONS = {
  ...
  // The identity is a property of the machine, not of a repository: it has to
  // be settable from the welcome screen, before any repository is open, because
  // that is where a user with a fresh git lands.
  setIdentity: async (body) => ({
    ok: true,
    command: `git config --global user.name "${body.name}" && git config --global user.email "${body.email}"`,
    identity: await git.setIdentity(body, { global: body.global !== false }),
  }),
};

/**
 * Adds a readable reason to a failed action.
 *
 * Without this the renderer sees no `message` in the response and falls back to
 * "Request failed (HTTP 422)", which is what a user got when a commit failed
 * because git had no identity configured. Every action now answers with prose
 * and, where the app can fix the cause itself, a flag saying so.
 */
function annotate(result, action) {
  const payload = {
    ok: false,
  ...
  const explained = explainGitFailure(result, { action });
  payload.error = explained.code;
  payload.message = explained.message;
  payload.hint = explained.hint;
  if (explained.needsIdentity) payload.needsIdentity = true;
  return payload;
}
...
actionRouter.post('/action', asyncRoute(async (req, res) => {
  ...
  if (REPO_FREE_ACTIONS[action]) {
    let result;
    try {
      result = await REPO_FREE_ACTIONS[action](req.body || {});
    } catch (error) {
      // Same shape as the repo-scoped branch below: the renderer shows the
      // message, so it must not arrive as a generic internal error.
    ...
    if (!result.ok) {
      res.status(422).json({ action, ...annotate(result, action) });
      return;
    }
  ...
  if (!requested) {
    res.status(400).json({ error: 'missing_path', message: 'No repository is open for this action.' });
    return;
  }

  let found;
  try {
    found = findRepositoryRoot(requested);
  } catch (error) {
  ...
  if (!result.ok) {
    res.status(422).json({ action, ...annotateCredentialFailure(annotate(result, action)) });
    return;
  }
