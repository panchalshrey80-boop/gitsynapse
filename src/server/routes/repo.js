...
import { forgetRepo, rememberRepo } from '../store.js';
...
/**
 * Forgets the open repository. Nothing on disk changes — this only stops the
 * app reopening it on the next launch, which is what "Close repository" means
 * to someone who wants to start somewhere else.
 */
repoRouter.post('/repo/close', (req, res) => {
  forgetRepo(typeof req.body?.path === 'string' ? req.body.path : null);
  res.json({ ok: true });
});
...
repoRouter.get('/fs/list', (req, res) => {
  let target;
  try {
    // No path means "start at home", which is the only default that makes sense
    // for a picker someone is browsing by eye.
    target = expandUserPath(req.query.path ? String(req.query.path) : os.homedir());
  } catch (error) {
    res.status(400).json({ error: error.code || 'invalid_path', message: error.message });
    return;
  }
  ...
  try {
    ...
    // A permission error is the common one on macOS, and it deserves advice
    // rather than "Cannot read that folder: undefined", which is what an
    // unhandled throw produced here.
    const status = error.code === 'not_found' ? 404 : error.code === 'permission_denied' ? 403 : 400;
    res.status(status).json({ error: error.code || 'unreadable', message: error.message });
    return;
  }
});
...
repoRouter.post('/repo/open', asyncRoute(async (req, res) => {
  let requested;
  try {
    requested = expandUserPath(req.body?.path);
  } catch (error) {
    res.status(400).json({ error: error.code || 'missing_path', message: error.message });
    return;
  }
  ...
  if (found?.bare) {
    res.status(400).json({
      error: 'bare_repository',
      message: 'That is a bare repository: it stores history but has no working tree, so there '
        + 'are no files to stage, commit or diff. Open a normal checkout of it instead.',
    });
    return;
  }

  if (!found) {
    // Say what is actually wrong. "Not a git repository" for a folder that does
    // not exist, or that macOS is blocking, sends people looking in the wrong
    // place entirely.
    if (!fs.existsSync(requested)) {
      res.status(404).json({ error: 'not_found', message: `There is no folder at ${requested}.` });
      return;
    }
    if (!isDirectory(requested)) {
      res.status(400).json({ error: 'not_a_directory', message: `${requested} is a file, not a folder.` });
      return;
    }

    res.status(404).json({
      error: 'not_a_repository',
      message: `${requested} is not a git repository yet.`,
      // The renderer offers a one-click "initialise here" from this.
      canInitialise: true,
      path: requested,
    });
    return;
