...
async function request(path, { method = 'GET', body, signal } = {}) {
  ...
  if (!response.ok) {
    // A failure must never be reported as "Request failed (HTTP 422)". If the
    // server sent no prose, fall back to git's own output and then to the
    // command that ran, so there is always something to act on.
    const gitOutput = String(payload.stderr || payload.stdout || '').split('\n').find((line) => line.trim());
    const fallback = gitOutput
      ? gitOutput.replace(/^(fatal|error):\s*/i, '').trim()
      : `That did not work${payload.command ? `: ${payload.command}` : ` (HTTP ${response.status})`}.`;
...
export const api = {
  ...
  closeRepo: (path) => request('/api/repo/close', { method: 'POST', body: { path: path || null } }),
  gitIdentity: (path) => request(`/api/git/identity${path ? `?path=${encodeURIComponent(path)}` : ''}`),
