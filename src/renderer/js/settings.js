...
import { identityFields } from './identity.js';
...
export async function openSettings(section = 'ai') {
  ...
  /* --- Git identity --- */
  // Loaded as it is, so the dialog shows the truth about what a commit will be
  // attributed to rather than an empty box the user has to guess about.
  const identity = await api.gitIdentity(state.repoPath).catch(() => ({ name: '', email: '', configured: false }));
  const identityInputs = identityFields(identity);
  ...
  body.append(
    ...
    h('div.card', {}, [
      h('div.card__head', {}, [
        h('div.card__title', { text: 'Commits are attributed to' }),
        h('div.row__meta', {
          text: identity.configured
            ? (identity.source === 'global' ? 'from your global git config' : 'from this repository')
            : 'not set yet — git will refuse to commit',
        }),
      ]),
      h('div.card__body', {}, [
        identityInputs.field,
        identityInputs.emailField,
      ]),
    ]),
    h('div.card', {}, [
  ...
  try {
    ...
    // The identity is saved first: if git has none, nothing else in this dialog
    // matters as much, and a rejected email should not silently discard the
    // provider settings the user just typed.
    const wanted = identityInputs.values();
    const changed = wanted.name !== (identity.name || '') || wanted.email !== (identity.email || '');
    if (changed && (wanted.name || wanted.email)) {
      if (!wanted.name || !wanted.email) {
        throw new Error('Fill in both your name and your email — git needs the pair.');
      }
      const saved = await api.action('setIdentity', wanted);
      identity.name = saved.identity?.name || wanted.name;
      identity.email = saved.identity?.email || wanted.email;
    }
